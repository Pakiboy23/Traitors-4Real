...
const getAllowedOrigins = () => {
  ...
  if (process.env.FUNCTIONS_EMULATOR === "true") return EMULATOR_ALLOWED_ORIGINS;
  return [];
};
...
export const image = onRequest({ cors: false, secrets: ["GENAI_API_KEY"] }, async (req, res) => {
  ...
  const prompt = (req.body?.prompt || "").toString();
  const size = (req.body?.size || "1K").toString() as "1K" | "2K" | "4K";
  if (!prompt) {
    res.status(400).json({ error: "Missing prompt" });
    return;
  }
...
export const generateCastPortraits = onCall(
  { secrets: ["GENAI_API_KEY"], timeoutSeconds: 300, memory: "1GiB" },
  async (request) => {
    ...
    await docRef.set(
      { state: { ...state, castStatus }, updatedAt: FieldValue.serverTimestamp() },
      { merge: true }
    );
...
export const ensureCastPortraits = onCall(
  { secrets: ["GENAI_API_KEY"], timeoutSeconds: 300, memory: "1GiB" },
  async (request) => {
    const email = request.auth?.token?.email;
    if (!email || !ADMIN_EMAILS.includes(String(email).toLowerCase())) {
    ...
    try {
      ...
      const missingAfter = CAST_NAMES.filter(
        (name) => !castStatus[name]?.portraitUrl
      );
